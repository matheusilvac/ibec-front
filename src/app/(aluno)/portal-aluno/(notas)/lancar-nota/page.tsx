"use client"
import {NativeSelect, NumberInput, Portal, Select, Stack, Table, Tabs} from "@chakra-ui/react";
...
export default function LancarNotaPage() {
    ...
    const handleClickSetTurma = (id: number) => {
        const {token} = parseCookies();
        axios.get(`https://portal-aluno-ibec-cgdhfngvhfb2g3f6.canadacentral-01.azurewebsites.net/api/admin/turmas/${id}/alunos`, {
            headers: {Authorization: `Bearer ${token}`}
        })
            .then((response) => {
        ...
        if (idApostila) {
            axios.get(`https://portal-aluno-ibec-cgdhfngvhfb2g3f6.canadacentral-01.azurewebsites.net/api/admin/prova/apostila/${idApostila}`, {
                headers: {Authorization: `Bearer ${token}`}
            })
                .then((response) => {
    ...
    const handleClickSetNota = (id: string) => {
        ...
        console.log(payload)
        axios.post(`https://portal-aluno-ibec-cgdhfngvhfb2g3f6.canadacentral-01.azurewebsites.net/api/admin/nota`,
            payload,
            {
                headers: {Authorization: `Bearer ${token}`}
            })
                    ...
                    <Table.Root size="lg" interactive>
                        <Table.Header className="rounded-lg">
                            <Table.Row className="bg-white text-black font-medium rounded-lg">
                                ...
                                <Table.ColumnHeader className="text-black">Prova</Table.ColumnHeader>
                                <Table.ColumnHeader className="text-black">Nota</Table.ColumnHeader>
                        ...
                        <Table.Body>
                            {alunos.map((aluno) => (
                                <Table.Row key={aluno.id}
                                    ...
                                    <Table.Cell>
                                        <select className="bg-gray-100 pl-2 py-2 rounded-lg"
                                                onChange={(e) => setProvaId(e.currentTarget.value)}>
                                            <option value="">Selecione a prova</option>
                                            {provas.map((prova) => (
                                                <option value={prova.id} key={prova.id}
                                                        className="bg-gray-100 pl-2 rounded-lg">{prova.nome}</option>
                                            ))}
                                        </select>
                                    </Table.Cell>
                                    <Table.Cell>
